...
int main()
{
    ...
    // a : 덧붙여 쓰기 전용 (append mode) : 파일이 없으면 생성
    // r+ : 읽기/쓰기 다 가능 : 파일이 없으면 에러
    // w+ : 읽기/쓰기 다 가능 : 파일이 없으면 생성
    // a+ : 읽기/덧붙여 쓰기 다 가능 : 파일이 없으면 에러
    ...
    fclose(fp1); // FILE Stream 의 버퍼에 등록되어있는 해당 파일을 등록 해제시키는 함수

    FILE* fp2 = fopen("b.txt", "r");

    if (fp2 == NULL) {
        printf("failed to open b.txt");
        exit(1);
    }
    ...
    // 파일을 생성하지 못했다는 의미
    fclose(fp2); // -> 버퍼에서 해제됨
    // 쓰기모드에서 버퍼에 들어있는 파일의 내용 수정이 끝나자마자 fclose 를 호출해주는 것이 안전함
    ...
    if (fp4 == NULL) {
        printf("failed to open chulsu.txt");
        exit(1);
    }
    ...
    if (fp5 == NULL) {
        printf("failed to open youngheui.txt");
        exit(1);
    }
